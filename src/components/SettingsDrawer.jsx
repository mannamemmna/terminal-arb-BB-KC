...
export default function SettingsDrawer({ open, onClose }) {
  ...
  useState(() => {
    fetch(`${getBackend()}/api/auth/status`, { credentials: 'include' })
      .then(r => r.json())
      .then(d => { setAuthenticated(d.authenticated); setChecking(false); })
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full sm:w-80 bg-bg-panel border-l border-border z-50 shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-3 sm:px-4 py-3 border-b border-border shrink-0">
          ...
          <button onClick={onClose} className="text-text-dim hover:text-text-primary cursor-pointer text-lg leading-none">✕</button>
        </div>
        ...
        {!checking && !authenticated && (
          <div className="flex-1 flex items-center justify-center px-6">
            <div className="w-full max-w-xs">
              ...
              <button onClick={handleLogin} className="w-full mono text-xs font-bold uppercase bg-accent-blue/10 border border-accent-blue/30 text-accent-blue rounded px-3 py-2 cursor-pointer hover:bg-accent-blue/20 transition-colors">
                Unlock
              </button>
            </div>
          </div>
        ...
        {checking && (
          <div className="flex-1 flex items-center justify-center">
            <div className="mono text-xs text-text-dim pulse-dot">Checking...</div>
          </div>
        )}

        {/* Settings content (authenticated only) */}
        {authenticated && (
          <div className="flex-1 overflow-auto px-3 sm:px-4 py-3 space-y-4 sm:space-y-5">
            <div>
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-2">
                <button onClick={() => killSwitch('pause')} className={`mono text-xs font-bold px-3 py-1.5 rounded border cursor-pointer ${killState === 'PAUSED' ? 'text-accent-amber border-accent-amber/40 bg-accent-amber/5' : 'text-text-dim border-border-light bg-transparent'}`}>⏸ Pause</button>
                <button onClick={() => killSwitch('resume')} className={`mono text-xs font-bold px-3 py-1.5 rounded border cursor-pointer ${killState === 'ACTIVE' ? 'text-accent-green border-accent-green/40 bg-accent-green/5' : 'text-text-dim border-border-light bg-transparent'}`}>▶ Resume</button>
                <button onClick={() => killSwitch('close-all')} className="mono text-xs font-bold px-3 py-1.5 rounded border text-accent-red border-accent-red/40 bg-accent-red/5 cursor-pointer">🛑 Close All</button>
              </div>
            ...
            <div>
              ...
              <div className="mt-1.5 grid grid-cols-2 sm:grid-cols-2 gap-1">
                {ALL_SYMBOLS.map(sym => (
